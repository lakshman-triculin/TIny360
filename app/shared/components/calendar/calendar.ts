...
export class Calendar implements AfterViewInit,OnInit,OnDestroy,ControlValueAccessor {
    ...
    onInputFocus(inputfield:any, event:any) {
        this.focus = true;
        if(this.showOnFocus) {
    ...
    parseDate(value:any, format:any) {
		...
		let iFormat:any, dim:any, extra:any,
		iValue:any = 0,
		shortYearCutoff:any = (typeof this.shortYearCutoff !== "string" ? this.shortYearCutoff : new Date().getFullYear() % 100 + parseInt(this.shortYearCutoff, 10)),
		year :any= -1,
		month:any = -1,
		day :any= -1,
		doy :any= -1,
