...
export class Dialog implements AfterViewInit,AfterViewChecked,OnDestroy {
    ...
    ngAfterViewInit() {
        ...
        if(this.draggable) {
            this.documentDragListener = this.renderer.listenGlobal('body', 'mousemove', (event:any) => {
                this.onDrag(event);
            });
        }
        
        if(this.resizable) {
            this.documentResizeListener = this.renderer.listenGlobal('body', 'mousemove', (event:any) => {
                this.onResize(event);
            });
            
            this.documentResizeEndListener = this.renderer.listenGlobal('body', 'mouseup', (event:any) => {
                if(this.resizing) {
                    this.resizing = false;
                }
            });
        ...
        if(this.responsive) {
            this.documentResponsiveListener = this.renderer.listenGlobal('window', 'resize', (event:any) => {
                this.center();
            });
        }
